		...
		while (i < masterdb.size()) {
			...
			if (currentsecurity.getCouponpaymentdate() != null) {
				...
				if (dcc.equalsIgnoreCase("a/360")) {
					...
					couponincome = couponincome
							+ (double) ((qty + bonusqty - sellqty) * facevalue * 0.01 * couponrate * netdays) / 360;
				}
				if (dcc.equalsIgnoreCase("a/365")) {

					couponincome = couponincome
							+ (double) ((qty + bonusqty - sellqty) * facevalue * 0.01 * couponrate * netdays) / 365;
				}
				if (dcc.equalsIgnoreCase("a/366")) {
					couponincome = couponincome
							+ (double) ((qty + bonusqty - sellqty) * facevalue * 0.01 * couponrate * netdays) / 366;
				}
				if (dcc.equalsIgnoreCase("30/360")) {
					...
					int a = (month * 30) + day;
					couponincome = couponincome
							+ (double) ((qty + bonusqty - sellqty) * facevalue * 0.01 * couponrate * a) / 360;
				}
			}
			allcouponincomes.put(securityname, couponincome);
			i++;
